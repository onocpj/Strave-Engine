...
namespace Strave {
	...
	public:
		...
		*/
		template<typename Type> void Draw(const Type& obj) const;															// critical function (runtime function)
		...
		template<> inline void Draw<sf::CircleShape>(const sf::CircleShape& obj) const { m_RenderWindow->draw(obj); }			// critical function (runtime function)
		inline void Draw(const sf::Vertex* vertices, std::size_t vertexCount, sf::PrimitiveType type) const { m_RenderWindow->draw(vertices, vertexCount, type); }	// critical function (runtime function)
		/**
