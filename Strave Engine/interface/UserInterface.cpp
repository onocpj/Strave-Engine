...
namespace Strave 
{
	...
	namespace GUI 
	{
		...
		UserInterface::UserInterface()
			: GameObject2D(),
			m_Mouse(UNDEF_PTR), m_ShowState(GUI_INIT_DEF_VIEW_STATE), m_WindowReference(UNDEF_PTR)
		{
		...
		Cell::Element* Cell::Element::Create(const Cell& cell) {
			...
			t_ElementSize = cell.GetRectSpriteSize() * (GUI_ELEMENT_MARGINS_PERCENTAGE / 100.0f);
			t_ElementPosition = {
				cell.GetSpritePosition().x + cell.GetRectSpriteSize().x - t_ElementSize.x,
				cell.GetSpritePosition().y + cell.GetRectSpriteSize().y - t_ElementSize.y,
			};
		...
		Grid::Grid(const Strave::Vector2u& dimensions, sf::Texture& cellTexture, std::weak_ptr<sf::Texture> tableTexture, std::weak_ptr<sf::Sprite> sprite, std::weak_ptr<sf::RectangleShape> rectSprite, const Strave::Vector2f& position, const Strave::Vector2f& spriteSize)
			...
			m_RowsNumber(dimensions.x), m_ColumnsNumber(dimensions.y), m_Cell(UNDEF_STD_VECTOR), m_MarginsSize(GUI_NO_MARGINS)
		{
		...
		void Grid::SetMargins(const Strave::Vector2f& margins) {
			...
			t_RowCounter = GUI_UINT_BEGINING;
			t_ColumnCounter = GUI_UINT_BEGINING;
			...
			t_CellSize = {
				m_Cell.at(GUI_VECTOR_BEGINING)->GetRectSpriteSize().x - t_SnipSize.x,
				m_Cell.at(GUI_VECTOR_BEGINING)->GetRectSpriteSize().y - t_SnipSize.y
			};
			...
			t_TablePosition = {
				this->GetSpritePosition().x + margins.x,
				this->GetSpritePosition().y + margins.y
			};
			...
			for (std::vector<std::shared_ptr<Cell>>::iterator iter = m_Cell.begin(); iter != m_Cell.end(); iter++) {
				...
				if (t_ColumnCounter < m_ColumnsNumber) {
					...
					t_ColumnCounter = GUI_UINT_BEGINING; 		// Reset value, that represents cells in one row
		...
		void Grid::SetGridPosition(const Strave::Vector2f& position) {
			...
			t_CellSize = {
				m_Cell.at(GUI_VECTOR_BEGINING)->GetRectSpriteSize().x,
				m_Cell.at(GUI_VECTOR_BEGINING)->GetRectSpriteSize().y
			};
		...
		bool Grid::MouseDrag(void) {
			...
			if (m_ShowState == true) {
				...
				if (t_Dragable) {
					this->SetGridPosition(this->GetSpritePosition());
					return t_Dragable;
				}
			}
