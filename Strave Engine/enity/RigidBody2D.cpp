...
namespace Strave {
	...
	bool RigidBody2DContainer::IsEmpty(void) {
		...
		if (m_ObjectContainer.size() == 0) {
			return RIGIDBODY_CONTAINER_2D_EMPTY;

		} else {
			return RIGIDBODY_CONTAINER_2D_NOT_EMPTY;
		}
	...
	RigidBody2D::RigidBody2D(OBJECT_TYPE objectType, std::weak_ptr<sf::Texture> texture, std::weak_ptr<sf::Sprite> sprite, std::weak_ptr<sf::RectangleShape> rectSprite, const Strave::Vector2f& position, const Strave::Vector2f& spriteScale)
		...
		m_RigidBodyEnabled(RIGIDBODY_ENABLED), m_InCollision(NOT_IN_COLLISION)
	{
	...
	void RigidBody2D::Enable(RigidBody2D& obj) {
		...
		if (!obj.RigidBodyEnabled()) {
			...
			obj.EnableRigidBody(RIGIDBODY_ENABLE);			// Set rigibody state to enabled
	...
	void RigidBody2D::Disable(RigidBody2D& obj) {
		...
		if (obj.RigidBodyEnabled()) {
			...
			obj.EnableRigidBody(RIGIDBODY_DISABLE);			// Set rigibody state to disabled
		...
		for (std::vector<RigidBody2D*>::iterator iter = RigidBody2DContainer::m_ObjectContainer.begin(), iterEnd = RigidBody2DContainer::m_ObjectContainer.end(); iter != iterEnd; iter++) {
			...
			for (std::vector<RigidBody2D*>::iterator iter2 = RigidBody2DContainer::m_ObjectContainer.begin(); iter2 != RigidBody2DContainer::m_ObjectContainer.end(); iter2++) {
				...
				if ((*iter) != (*iter2)) {
					...
					if (Collider::BoundingBoxTest(*((*iter)->GetModel<sf::Sprite>()), *((*iter2)->GetModel<sf::Sprite>())) == IN_COLLISION) {
						(*iter)->SetCollisionStatus(IN_COLLISION);	// Set iter collision status to true
						(*iter2)->SetCollisionStatus(IN_COLLISION);	// Set iter2 collision status to true
						...
						if ((*iter)->InCollision()) {
							...
							(*iter)->SetCollisionStatus(NOT_IN_COLLISION);
						}
						// If iter2 is in collision
						if ((*iter2)->InCollision()) {
							// Set iter2 collision status to false
							(*iter2)->SetCollisionStatus(NOT_IN_COLLISION);
						}
					}
				}
			}
		...
		if (obj.RigidBodyEnabled()) {
			...
			for (std::vector<RigidBody2D*>::iterator iter = RigidBody2DContainer::m_ObjectContainer.begin(), iterEnd = RigidBody2DContainer::m_ObjectContainer.end(); iter != iterEnd; iter++) {
				...
				if ((*iter) != &obj) {
					...
					if (Collider::BoundingBoxTest(*(obj.GetModel<sf::Sprite>()), *((*iter)->GetModel<sf::Sprite>())) == IN_COLLISION) {
						// Set object collision status to true
						obj.SetCollisionStatus(IN_COLLISION);
						...
						if (obj.InCollision()) {
							...
							obj.SetCollisionStatus(NOT_IN_COLLISION);
						}
